...
const Restocking = () => {
  const { restockings, isLoading, createRestocking, updateRestocking, deleteRestocking } = useRestockings();
  ...
  const filteredRestockings = restockings.filter(restocking => {
    ...
    const matchesSearch = restocking.reference_number?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleDeleteRestocking = async (restockingId: string) => {
    ...
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce réapprovisionnement ?')) {
      try {
  ...
  const handleStatusChange = async (restockingId: string, newStatus: string) => {
    const restocking = restockings.find(r => r.id === restockingId);
  ...
  const handleSaveRestocking = async (restockingData: any) => {
    try {
      if (formMode === 'create') {
        const newRestocking = await createRestocking.mutateAsync(restockingData);
        await logAction.mutateAsync({
          action_type: 'create',
          entity_type: 'restocking',
        ...
        await updateRestocking.mutateAsync({ id: selectedRestocking.id, ...restockingData });
        await logAction.mutateAsync({
          action_type: 'update',
          entity_type: 'restocking',
          entity_id: selectedRestocking.id,
          entity_name: selectedRestocking.reference_number || 'Réapprovisionnement',
  ...
  const stats = {
    ...
    pendingRestockings: restockings.filter(r => r.status === 'pending').length,
    completedRestockings: restockings.filter(r => r.status === 'completed').length,
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-muted-foreground">Chargement des réapprovisionnements...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Réapprovisionnements</h1>
          <p className="text-muted-foreground">Gérez vos réapprovisionnements</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-foreground">{stats.totalRestockings}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-yellow-600">{stats.pendingRestockings}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600">{stats.completedRestockings}</p>
                  ...
                  <div>
                    <div className="flex items-center space-x-2 mb-1">
                      <h3 className="font-semibold text-foreground">
                        {restocking.reference_number || `RÉA-${restocking.id.slice(0, 8)}`}
                      </h3>
                    ...
                    <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span>{getRestockingItemsCount(restocking)} articles</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <Calendar className="w-4 h-4" />
                        <span>{new Date(restocking.created_at).toLocaleDateString('fr-FR')}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-4">
                  <div className="text-right">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {new Date(restocking.created_at).toLocaleTimeString('fr-FR', { 
                        hour: '2-digit', 
                        minute: '2-digit' 
                      })}
                    </p>
                  ...
                  <div className="flex space-x-2">
                    <Button variant="ghost" size="sm" onClick={() => handleViewRestocking(restocking)}>
                      <Eye className="w-4 h-4" />
                    </Button>
                    {restocking.status !== 'completed' && (
                      <Button variant="ghost" size="sm" onClick={() => handleEditRestocking(restocking)}>
                    ...
                    {restocking.status === 'pending' && (
                      <>
                        <Button 
                          ...
                          onClick={() => handleStatusChange(restocking.id, 'completed')}
                        >
                        ...
                        <Button 
                          ...
                          onClick={() => handleStatusChange(restocking.id, 'cancelled')}
                        >
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-foreground mb-2">
                {searchTerm || statusFilter !== 'all' ? 'Aucun réapprovisionnement trouvé' : 'Aucun réapprovisionnement'}
              </h3>
